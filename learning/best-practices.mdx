...
A few principles recur across everything below, and recognising them is more valuable than memorising the list:
...
- **Why.** Integrity lives in one place the application can't bypass. (The known 404-vs-500 reporting gap on this path is deliberately documented, not silently patched; see [Issues](/issues).)
...
- **Why.** `npm ci` re-runs only when manifests change, not on every code edit. The runner stage ships only `dist/` + prod deps, for a smaller image and lower attack surface ([DevOps](/operations/devops)).
