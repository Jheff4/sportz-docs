...
### Guard client-only state against hydration mismatch — without a `setState`-in-effect
The dark-mode toggle renders nothing until next-themes' `resolvedTheme` is defined.

- **Why.** The theme lives in `localStorage`, which the server can't read; rendering one icon on the server and another on the client is a **hydration mismatch** (server HTML ≠ first client render → React discards and re-renders, causing a flash). We *used* to gate on a `mounted` flag set in `useEffect`, but that's a `setState`-in-effect (see below) — so we gate on `resolvedTheme` instead, which next-themes resolves via *its own* internal update. Same hydration safety, no effect we own.
