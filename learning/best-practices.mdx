...
### Guard client-only state against hydration mismatch
The dark-mode toggle renders nothing until a `mounted` flag flips in `useEffect`.

- **Why.** The theme lives in `localStorage`, which the server can't read. Rendering one icon on the server and another on the client is a hydration mismatch — deferring to after mount makes the first client render match the server's.
