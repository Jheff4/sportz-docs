...
- **Tests** — 95 Vitest tests: unit (validation, status), integration (routes against real Postgres), WebSocket (real socket pairs)
- **CI/CD** — three GitHub Actions workflows (lint+format, tests, docker build/push)
- **Containerization** — multi-stage Dockerfile, dev (Neon Local) and prod compose files
- **Deploy** — Render Blueprint (Docker runtime)
- **Frontend** — Next.js App Router app: match grid, commentary timeline, dark mode, animations, responsive bottom sheet, modals, skeletons
