...
- **Read-only and presence-based.** It never mutates production and never asserts exact data (real data changes) — only that things are *present* and *connected*. Generous timeouts absorb Render's free-tier cold start.
