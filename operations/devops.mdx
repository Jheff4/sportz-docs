...
The final image ships no compiler, no devDeps, no source, so it's smaller and has a lower attack surface. Both compose files target the `runner` stage (a mismatch here was a real bug, see [Issues](/issues)).
