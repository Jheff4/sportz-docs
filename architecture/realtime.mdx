...
The distinction matters: `match_created` is global (a new match should appear in everyone's grid), while `commentary` is room-scoped (only people watching match 5 care about match 5's commentary).
...
This entire mechanism is **in-process**. The `matchSubscribers` map lives in one Node process's memory. The moment you run a second API instance, a client connected to instance A will not receive an event created on instance B — because B's broadcast only reaches B's local map. The fix (a Redis pub/sub backbone) is documented as the first mandatory infrastructure addition in [System Architecture](/architecture/system). It is correctly **not** built yet.
