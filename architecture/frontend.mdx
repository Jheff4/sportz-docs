...
**Live matches sort to the top.** The displayed list is derived with a stable `compareLiveFirst` sort (over a *copy*, since sorting the cache array in place would mutate shared state during render), so live matches lead and, within each group, newest-first order is preserved.
