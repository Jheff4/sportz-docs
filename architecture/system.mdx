...
| **100k+** | — | Need a **pub/sub backbone** (Redis pub/sub or similar): each instance publishes broadcasts to a channel, every instance relays to its own local subscribers. This is the first piece of "missing" infrastructure that becomes mandatory. |

The key insight for an interview or a design review: **the synchronous in-process broadcast is not a flaw — it's the right choice until you horizontally scale the API.** Adding Redis before then is premature complexity.
