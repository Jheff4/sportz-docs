...
Logs (what happened), metrics (how it performs), traces (where a request went). Backend has all three: Winston for logs, New Relic APM for metrics and traces (auto-instruments express/pg/http). Frontend has the same three via New Relic Browser — wired in code, but pending Vercel env vars before it reports anything. PostHog covers a separate, fourth concern — product analytics — not one of the three pillars.
</Accordion>
...
<Accordion title="The backend uses New Relic, not OpenTelemetry. Why?">
